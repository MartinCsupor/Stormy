...
const getUVIndexAndDewPoint = async (lat, lon) => {
...
const getCoords = async (e) => {   
    ...
    const vilagtalalat = vilagvarosok
    .filter(v => v.toLowerCase().startsWith(varosInp.toLowerCase()))
    .slice(0,5);
    ...
    } else if (talalatok.length < 0 && vilagtalalat.length > 0) {
        if (varosInp.length ==  0){
            talalatok.innerHTML = ""
            return
        }
        vilagtalalat.forEach(varos => {
            const div = document.createElement("div")
            div.classList.add("bg-[#82B3DB]", "hover:bg-[#476D98]", "rounded-lg", "px-2", "py-1")
            div.textContent = varos
...
const renderWeather = async (telepules) => {
    ...
    nyugtatext1.textContent = `${String(new Date(dailyWeather.sys.sunset * 1000).getHours()).padStart(2, "0")}:${String(new Date(dailyWeather.sys.sunset * 1000).getMinutes()).padStart(2, "0")}`
...
const renderHourlyWeather = async (telepules) => {
    ...
    osszesitett.forEach((nap)=>{
        ...
        napNev.innerHTML = nap.nap.charAt(0).toUpperCase() + nap.nap.slice(1);
        napEso.innerHTML = Math.round(nap.adatok.eso) + "%";
        napIkon.src = `images/icons/weather/${nap.adatok.ikon}.svg`;
        napIkon.classList.add("mx-auto", "h-7")
        napMin.innerHTML = hofok(nap.adatok.min) + getUnitSymbol();
        napMax.innerHTML = hofok(nap.adatok.max) + getUnitSymbol();
